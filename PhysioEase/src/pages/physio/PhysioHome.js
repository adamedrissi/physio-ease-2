...
import '../../styles/physio/PhysioHome.css';
...
function Home() {
  ...
  return (
    <div>
      ...
      {user.speciality && (
        <p>{t('yourSpeciality', { speciality: t(`specialities.${user.speciality}`) })}</p>
      )}

      <Link to="/track-progress">
        <button className="track-button">{t('trackPatientsHere')}</button>
      </Link>
    </div>
  );
}
